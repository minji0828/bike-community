...
- 현재 MVP1 정책 기준 신규 코스 생성은 인증 사용자만 허용한다.
- 따라서 신규 생성 코스의 소유권은 `owner_user_id`가 기준이며, `device_uuid`는 설치 단위 추적/레거시 호환 보조 필드로 본다.
...
### 2.4 course_highlights

역할: 코스 위 실제 경험 포인트(전망/보급/주의/사진 스팟)를 좌표와 함께 저장한다.

- id (PK)
- course_id (FK -> courses.id)
- author_user_id (not null)
- type (enum string: viewpoint/restroom/water/cafe/danger/photo/note)
- title (nullable)
- note (nullable)
...
- btree(author_user_id)
- GiST(location)

메모:

- Phase 1에서는 moderation API 없이 `hidden=false` 상태로 생성한다.
- 조회는 공개 하이라이트 + 작성자 본인 private 하이라이트를 함께 반환한다.

### 2.5 course_collections

역할: 여러 코스를 주말 여행/장거리 라이딩 단위로 묶는 사용자 컬렉션.
...
- btree(owner_user_id, updated_at desc)
- btree(visibility, updated_at desc)

### 2.6 course_collection_items

역할: 컬렉션에 담긴 코스와 순서를 관리한다.

- id (PK)
- collection_id (FK -> course_collections.id)
- course_id (FK -> courses.id)
- position_index (not null)
- created_at

인덱스/제약(권장)

- unique(collection_id, course_id)
- btree(collection_id, position_index)
...
erDiagram
  ...
  users ||--o{ course_collections : owns
  courses ||--o{ course_tags : tagged
  tags ||--o{ course_tags : uses
  courses ||--o{ course_warnings : warns
  courses ||--o{ course_highlights : highlights
  course_collections ||--o{ course_collection_items : contains
  courses ||--o{ course_collection_items : listed
```
