...
- 대부분은 “자연어로 요청”만 해도 자동으로 동작하지만, 필요하면 **명령 접두사**로 모드를 명시할 수 있습니다.

---

## 2) OMX 명령어 체계

OMX에서 **현재 기준으로 구분해야 할 핵심은 이 두 가지**입니다.

- **스킬(workflow skill)**: `$이름` 형태가 기본이다. 예) `$ralph`, `$plan`, `$autopilot`
- **프롬프트(agent prompt)**: `/prompts:이름` 형태다. 예) `/prompts:architect`, `/prompts:executor`

추가로, `/skills` 는 사용 가능한 스킬을 둘러보는 명령이다.
- **주의:** `$name`, `/prompts:name` 에서 `name` 은 실제 명령어가 아니라 **자리표시자**다.

> 검증 기준: 로컬 OMX 설정(`~/.codex/config.toml`)의 `invocation_conventions`에는 **`$name` = skill**, **`/prompts:name` = custom prompt**로 명시돼 있다.
>
> 참고: 일부 오래된 스킬 문서에는 `/ralph`, `/plan` 같은 **레거시 슬래시 표기**도 남아 있다. 다만 혼동을 줄이기 위해 이 문서에서는 **기본 표기를 `$skill`** 로 통일한다.

## 3) 스킬 목록 (`$name`)

> 2026-03-06 재검증: 디스크에서 **일반 스킬 40개**(`~/.agents/skills`), **시스템 스킬 2개**(`~/.codex/skills/.system`), **프롬프트 32개**(`~/.codex/prompts`)를 직접 확인했다. 이 문서에 적은 명령어 중 실제 파일이 없는 항목은 제거한 상태다.
...
- **스킬 목록 보기**: `/skills`
- **레거시 호환 표기**: 일부 환경에서는 `/ralph`, `/plan` 등이 동작할 수 있지만, 이 문서에서는 **현재 기준 표기인 `$skill`** 사용을 권장한다.

---

## 8) 이 저장소에서 자주 쓰는 Gradle 명령(참고)
